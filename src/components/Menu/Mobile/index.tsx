...
import { Humburger, WrapperMenu, Line, Nav, ContentWrapper } from './styled-components';
...
function MobileMenu(): JSX.Element {
  ...
  const { offsetTop } = useSelector<IStore, IHooksState>((state) => state.hooks);
  const settingMenu = {
        ...
        <Nav>
          {menuItems.map((el, idx) => (
            <CustomMobileLink
              ...
              animatedLink={isOpen}
              style={{
                animationDelay: `${(idx + animationStartOffset) * animationDelay}s`,
              }}
            />
          ))}
        </Nav>
